...
conn.close()
scene_ids, entity_names, description_ids, answers, objdescriptions, locdescriptions, turk_answered = zip(*results)
...
def get_error_counts(descriptions, description_ids, scene_ids, entity_names, turk_answered, 
    ...
    for original,did,s,e,ta in zip(descriptions,description_ids,scene_ids,entity_names,turk_answered):
        ...
        if split_parse and re.search(to_split,u) is not None:
            ...
            if len(u_parseable) > 0:
                parseable.append((scene, speaker, (entity,original,u,ta,did,u_parseable)))
            else:
                unparseable.append((scene, speaker, (entity,original,u,ta,did,[])))
                error_counts['reagent_cant_parse']+=1
        else:
            parses = parse(u)
            if len(parses) > 0 and isinstance(parses[0].current[0],parse_goal):
                parseable.append((scene, speaker, (entity,original,u,ta,did,[parses[0].current[0]])))
            else:
                error_counts['reagent_cant_parse']+=1
                unparseable.append((scene, speaker, (entity,original,u,ta,did,[])))
...
# print 'Initial'
# oerror_counts1, oparseable, ounparseable = get_error_counts(descriptions_, description_ids, scene_ids, entity_names, turk_answered, original_oov, t_lexicon, t_structicon, parse_goal)
...
# oerror_counts5, oparseable, ounparseable = get_error_counts(stripped, description_ids, scene_ids, entity_names, turk_answered,  modified_oov, extended_t_lexicon, extended_t_structicon, parse_goal, split_parse=True, to_replace=to_remove)
...
print 'Initial'
# lerror_counts1, lparseable, lunparseable = get_error_counts(descriptions_, description_ids, scene_ids, entity_names, turk_answered, original_oov, t_lexicon, t_structicon, parse_goal)
...
# americanized = [re.sub('centre','center',re.sub('colour','color',u)) for u in stripped]
print 'Splitting'
lerror_counts5, lparseable, lunparseable = get_error_counts(stripped, description_ids, scene_ids, entity_names, turk_answered, modified_oov, extended_t_lexicon, extended_t_structicon, parse_goal, split_parse=True, to_replace=to_remove)
...
dids = [did for sc,sp,(e,o,m,b,did,ps) in lparseable if b]
