...
def kde(data, N=None, MIN=None, MAX=None, weights=None):
    ...
    if MIN is None or MAX is None:
        ...
        MIN = minimum - Range/2 if MIN is None else MIN
        MAX = maximum + Range/2 if MAX is None else MAX
    ...
    guess = 0.05
    try:
        # print fixed_point(0,M,I,SqDCTData),fixed_point(guess,M,I,SqDCTData)
    ...
    def pdf(value):
        if not isinstance(value, np.ndarray):
            value = [value]
        inds = np.searchsorted(mesh,value)
        inds[np.where(inds==len(mesh))]=len(mesh)-1
        return density[inds]
    return pdf
