...
class ContainmentFeature(Feature):
	...
	def measure(perspective, landmark, trajector):
		if isinstance(trajector, Landmark):
			return landmark.representation.contains( 
...
featureDict = {'distance':   DistanceFeature(),
			   ...
			   'in_front_of':OrientationFeature(Vec2(0,-1)), # in front
			   'behind':     OrientationFeature(Vec2(0,1)),  # behind
			   'left_of':    OrientationFeature(Vec2(-1,0)), # left
			   'right_of':   OrientationFeature(Vec2(1,0))   # right
			   }
